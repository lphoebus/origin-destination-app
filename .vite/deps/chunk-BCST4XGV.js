import {
  p
} from "./chunk-UISC6JT5.js";
import {
  i3 as i
} from "./chunk-KUWSTWZR.js";

// node_modules/@arcgis/core/views/interactive/interactiveToolUtils.js
function o(t) {
  return [t.on("before-add", (o3) => {
    const i3 = o3.item;
    if (null == i3 || t.includes(i3)) return i.getLogger("esri.views.interactive.interactiveToolUtils").warn("Tool is either already in the list of tools or tool is `null`. Not adding tool."), void o3.preventDefault();
    i3.onAdd();
  }), t.on("after-remove", (e) => {
    const t2 = e.item;
    t2.active && (t2.view.activeTool = null), t2.destroy();
  })];
}
function i2(e) {
  return e.visible && null != e.getEditableFlag && e.getEditableFlag(0) && e.getEditableFlag(1);
}
function n(e) {
  return "key-down" === e.type && e.key === p.cancel;
}

// node_modules/@arcgis/core/support/groundInstanceUtils.js
var n2 = Symbol("GroundInstance");
function o2(o3) {
  return null != o3 && "object" == typeof o3 && n2 in o3;
}

export {
  n2 as n,
  o2 as o,
  o as o2,
  i2 as i,
  n as n2
};
//# sourceMappingURL=chunk-BCST4XGV.js.map
