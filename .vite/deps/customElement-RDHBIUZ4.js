...
var idPrefix = "calcite-stepper-action";
var IDS = {
...
var StepBar = ({ disabled, active, complete, error }) => html`<svg class=${safeClassMap({
  [CSS.stepBar]: true
...
var _Stepper = class _Stepper extends LitElement {
  ...
  getEnabledStepIndex(startIndex, direction = "next") {
    ...
    while (newIndex >= 0 && newIndex < items.length && ((_a = items[newIndex]) == null ? void 0 : _a.disabled)) {
  ...
  handleDefaultSlotChange(event) {
    const items = slotChangeGetAssignedElements(event).filter((el) => (el == null ? void 0 : el.tagName) === "CALCITE-STEPPER-ITEM" && !isHidden(el));
    this.items = items;
  ...
  render() {
    ...
    this.el.role = "region";
    return html`<div class=${safeClassMap({ container: true, [CSS$1.singleView]: this.layout === "horizontal-single" })} ${ref(this.containerRef)}>${this.layout === "horizontal-single" && html`<div class=${safeClassMap({ [CSS$1.stepBarContainer]: true })}>${this.items.map((item, index) => StepBar({ active: index === this.currentActivePosition, complete: item.complete && index !== this.currentActivePosition && !item.error, disabled: item.disabled && index !== this.currentActivePosition, error: item.error && index !== this.currentActivePosition }))}</div>` || ""}${this.layout === "horizontal-single" && html`<div class=${safeClassMap({ [CSS$1.actionContainer]: true })}>${this.renderAction("start")}${this.renderAction("end")}</div>` || ""}<slot @slotchange=${this.handleDefaultSlotChange}></slot></div>`;
  }
  renderAction(position) {
    ...
    return layout === "horizontal-single" && !multipleViewMode ? html`<calcite-action alignment=center appearance=transparent class=${safeClassMap({
...
_Stepper.properties = { currentActivePosition: [16, {}, { state: true }], icon: [7, {}, { reflect: true, type: Boolean }], layout: [3, {}, { reflect: true }], messageOverrides: [0, {}, { attribute: false }], numbered: [7, {}, { reflect: true, type: Boolean }], numberingSystem: [3, {}, { reflect: true }], scale: [3, {}, { reflect: true }], selectedItem: [0, {}, { attribute: false }] };
_Stepper.styles = styles;
...
@esri/calcite-components/dist/components/calcite-stepper/customElement.js:
  ...
  v3.3.3 *)
*/
//# sourceMappingURL=customElement-RDHBIUZ4.js.map
