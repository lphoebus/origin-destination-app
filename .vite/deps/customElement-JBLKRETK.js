...
var _InputDatePicker = class _InputDatePicker extends LitElement {
    ...
    this.focusTrap = useFocusTrap({
      ...
      focusTrapOptions: {
        onActivate: () => {
          var _a;
          if (this.focusOnOpen) {
            (_a = this.datePickerEl) == null ? void 0 : _a.setFocus();
            this.focusOnOpen = false;
          }
        },
  ...
  set value(value) {
    ...
    const invalidValueCleared = value === "" && (((_a = this.startInputRef.value) == null ? void 0 : _a.value) !== "" || ((_b = this.endInputRef.value) == null ? void 0 : _b.value) !== "");
  ...
  onClose() {
    ...
    this.focusOnOpen = false;
    (_a = this.datePickerEl) == null ? void 0 : _a.reset();
  }
  ...
  keyDownHandler(event) {
    ...
    if (key === "Enter") {
      ...
      if (this.shouldFocusRangeEnd()) {
        (_a = this.endInputRef.value) == null ? void 0 : _a.setFocus();
      } else if (this.shouldFocusRangeStart()) {
        (_b = this.startInputRef.value) == null ? void 0 : _b.setFocus();
      }
      if (submitForm(this)) {
  ...
  formatNumerals(value) {
    return value ? value.split("").map((char) => {
      var _a, _b, _c, _d, _e;
  ...
  render() {
    ...
    })} .disabled=${disabled} .icon=${ICONS.calendar} .label=${this.range ? this.messages.startDate : this.messages.date} @calciteInputTextInput=${this.calciteInternalInputInputHandler} @calciteInternalInputTextBlur=${this.calciteInternalInputBlurHandler} @calciteInternalInputTextFocus=${this.startInputFocus} .placeholder=${(_a = this.localeData) == null ? void 0 : _a.placeholder} .readOnly=${readOnly} role=combobox .scale=${this.scale} .status=${this.status} ${ref(this.startInputRef)}></calcite-input-text>${!this.readOnly && !this.range && this.renderToggleIcon(this.open && this.focusedInput === "start") || ""}<span aria-hidden=true class=${safeClassMap(CSS.assistiveText)} id=${this.placeholderTextId ?? nothing}>${messages.dateFormat.replace("{format}", (_b = this.localeData) == null ? void 0 : _b.placeholder)}</span></div><div .ariaHidden=${!this.open} .ariaLabel=${messages.chooseDate} aria-live=polite aria-modal=true class=${safeClassMap(CSS.menu)} id=${this.dialogId ?? nothing} role=dialog ${ref(this.setFloatingEl)}><div class=${safeClassMap({
    ...
    })} .disabled=${disabled} .icon=${ICONS.calendar} .label=${this.messages.endDate} @calciteInputTextInput=${this.calciteInternalInputInputHandler} @calciteInternalInputTextBlur=${this.calciteInternalInputBlurHandler} @calciteInternalInputTextFocus=${this.endInputFocus} .placeholder=${(_c = this.localeData) == null ? void 0 : _c.placeholder} .readOnly=${readOnly} role=combobox .scale=${this.scale} .status=${this.status} ${ref(this.endInputRef)}></calcite-input-text>${!this.readOnly && this.layout === "horizontal" && this.renderToggleIcon(this.open) || ""}</div>` || ""}</div>${this.range && this.layout === "vertical" && html`<div class=${safeClassMap(CSS.verticalChevronContainer)}><calcite-icon .icon=${this.open ? ICONS.chevronUp : ICONS.chevronDown} .scale=${getIconScale(this.scale)}></calcite-icon></div>` || ""}</div>${HiddenFormInputSlot({ component: this })}${this.validationMessage && this.status === "invalid" ? Validation({ icon: this.validationIcon, id: IDS.validationMessage, message: this.validationMessage, scale: this.scale, status: this.status }) : null}` });
  }
...
_InputDatePicker.styles = styles;
...
@esri/calcite-components/dist/components/calcite-input-date-picker/customElement.js:
  ...
  v3.3.3 *)
*/
//# sourceMappingURL=customElement-JBLKRETK.js.map
