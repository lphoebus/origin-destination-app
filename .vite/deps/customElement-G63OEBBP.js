...
var _ColorPicker = class _ColorPicker extends LitElement {
  ...
  async load() {
    if (!this._valueWasSet) {
      this._value ?? (this._value = normalizeHex(hexify(DEFAULT_COLOR, this.alphaChannel)));
    }
  ...
  handleChannelBlur(event) {
    ...
    if (restoreValueDueToEmptyInput) {
      input.value = (_a = channels[channelIndex]) == null ? void 0 : _a.toString();
    }
  }
  ...
  handleChannelKeyUpOrDown(event) {
    ...
    if (key !== "ArrowUp" && key !== "ArrowDown" || !event.composedPath().some((node) => {
      var _a;
      return (_a = node.classList) == null ? void 0 : _a.contains(CSS.channel);
    })) {
      return;
    }
  ...
  updateCanvasSize(context = "all") {
    ...
    if (context === "all" || context === "color-field") {
      this.setCanvasContextSize((_a = this.colorFieldRenderingContext) == null ? void 0 : _a.canvas, dynamicDimensions.colorField);
    }
  ...
  renderChannel(value, index, ariaLabel, direction, suffix) {
    ...
    })} .suffixText=${suffix} .value=${value == null ? void 0 : value.toString()}></calcite-input-number>`);
  }
};
...
@esri/calcite-components/dist/components/calcite-color-picker/customElement.js:
  ...
  v3.3.3 *)
*/
//# sourceMappingURL=customElement-G63OEBBP.js.map
