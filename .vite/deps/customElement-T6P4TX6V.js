import {
  calciteSize48
} from "./chunk-JKGAOHUN.js";
import {
  CSS,
  XButton
} from "./chunk-R3Z5EHQ7.js";
import {
  getItemAncestors,
  getItemChildren,
  getLabel,
...
import {
  ...
  hideFloatingUI,
  reposition
} from "./chunk-XTUA4CHM.js";
import {
  debounce
} from "./chunk-CV3WX3PG.js";
import {
  toggleOpenClose
} from "./chunk-S3W7ND2A.js";
import {
  keyed
...
var _Combobox = class _Combobox extends LitElement {
  ...
  clickHandler(event) {
    ...
    if (composedPath.some((node) => {
      var _a;
      return (_a = node.classList) == null ? void 0 : _a.contains(CSS.button);
    })) {
      this.clearValue();
      event.preventDefault();
      return;
  ...
  hideChip(chipEl) {
    chipEl.classList.add(CSS2.chipInvisible);
  }
  showChip(chipEl) {
    chipEl.classList.remove(CSS2.chipInvisible);
  }
  ...
  setVisibleAndHiddenChips(chipEls) {
    ...
    chipEls.forEach((chipEl) => {
      if (chipEl.selected && !chipEl.classList.contains(CSS2.chipInvisible)) {
  ...
  scrollToActiveOrSelectedItem(scrollToSelected = false) {
    var _a;
    const item = scrollToSelected && ((_a = this.selectedItems) == null ? void 0 : _a.length) ? this.selectedItems[0] : this.keyboardNavItems[this.activeItemIndex];
    ...
    const stickyHeight = (stickyElement == null ? void 0 : stickyElement.offsetHeight) || 0;
  ...
  renderAllSelectedIndicatorChip() {
    ...
    return html`<calcite-chip class=${safeClassMap({
      [CSS2.chip]: true,
      [CSS2.chipInvisible]: !(this.allSelected && !selectedVisibleChipsCount),
      [CSS2.allSelected]: true
  ...
  renderSelectedIndicatorChip() {
    ...
    return html`<calcite-chip class=${safeClassMap({
      [CSS2.chip]: true,
      [CSS2.chipInvisible]: chipInvisible
  ...
  renderInput() {
    ...
    return html`<span class=${safeClassMap({
      [CSS2.inputWrap]: true,
      [CSS2.inputWrapSingle]: single
    })}>${showLabel && keyed("label", html`<span class=${safeClassMap({
      [CSS2.label]: true,
      [CSS2.labelIcon]: !!(selectedItem == null ? void 0 : selectedItem.icon)
      ...
      [CSS2.input]: true,
      [CSS2.inputSingle]: true,
      [CSS2.inputHidden]: showLabel,
      [CSS2.inputIcon]: this.showingInlineIcon && !!this.placeholderIcon
  ...
  renderFloatingUIContainer() {
    ...
    const classes = {
      [CSS2.listContainer]: true,
      [FloatingCSS.animation]: true,
      [FloatingCSS.animationActive]: open
    };
    const label = (this.filterText && ((_a = messages.add) == null ? void 0 : _a.replace("{text}", `${this.filterText}`))) ?? "";
    return html`<div aria-hidden=true class=${safeClassMap(CSS2.floatingUIContainer)} ${ref(setFloatingEl)}><div class=${safeClassMap(classes)} ${ref(setContainerEl)}><ul class=${safeClassMap({ [CSS2.list]: true, [CSS2.listHide]: !open })}>${this.selectAllEnabled && this.selectionMode !== "single" && this.selectionMode !== "single-persist" && html`<calcite-combobox-item class=${safeClassMap(CSS2.selectAll)} .id=${`${this.guid}-select-all-enabled-interactive`} .indeterminate=${this.indeterminate} .label=${messages.selectAll} .scale=${scale} .selected=${this.allSelected} tabindex=-1 text-label=${messages.selectAll ?? nothing} value=select-all ${ref(this.selectAllComboboxItemRef)}></calcite-combobox-item>` || ""}<slot></slot>${this.noMatchesFound && (this.allowCustomValues ? html`<li aria-label=${label ?? nothing} class=${safeClassMap(CSS2.noMatches)} @click=${this.customChipAddHandler} role=option tabindex=0>${highlightText({
      text: label,
      pattern: new RegExp(`(${escapeRegExp(this.filterText)})`, "i")
    })}</li>` : html`<li class=${safeClassMap({ [CSS2.noMatchesPlaceholder]: true, [CSS2.noMatches]: true })}>${messages.noMatches}</li>`) || ""}</ul></div></div>`;
  }
  renderSelectedOrPlaceholderIcon() {
    ...
    return this.showingInlineIcon && keyed("selected-placeholder-icon", html`<span class=${safeClassMap(CSS2.iconStart)}><calcite-icon class=${safeClassMap({
      [CSS2.selectedIcon]: !showPlaceholder,
      [CSS2.placeholderIcon]: showPlaceholder
  ...
  renderChevronIcon() {
    const { open } = this;
    return keyed("chevron", html`<span class=${safeClassMap(CSS2.iconEnd)}><calcite-icon class=${safeClassMap(CSS2.icon)} .icon=${open ? ICONS.chevronUp : ICONS.chevronDown} .scale=${getIconScale(this.scale)}></calcite-icon></span>`);
  }
  render() {
    var _a;
    ...
    const isClearable = !this.clearDisabled && this.selectionMode !== "single-persist" && !!((_a = this.value) == null ? void 0 : _a.length);
    return InteractiveContainer({ disabled: this.disabled, children: html`${this.labelText && InternalLabel({ labelText: this.labelText, onClick: this.onLabelClick, required: this.required, tooltipText: this.messages.required }) || ""}<div aria-live=polite class=${safeClassMap({
      [CSS2.wrapper]: true,
      [CSS2.wrapperSingle]: singleSelectionMode || !this.selectedItems.length,
      [CSS2.wrapperActive]: open
      ...
      [CSS2.gridInput]: true,
      [CSS2.selectionDisplayFit]: fitSelectionDisplay,
      [CSS2.selectionDisplaySingle]: singleSelectionDisplay
...
@esri/calcite-components/dist/components/calcite-combobox/customElement.js:
  ...
  v3.3.3 *)
*/
//# sourceMappingURL=customElement-T6P4TX6V.js.map
