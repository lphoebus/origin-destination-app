...
var StarIcon = ({ full, scale, partial }) => html`<calcite-icon class=${safeClassMap(partial ? void 0 : "icon")} .icon=${full ? "star-f" : "star"} .scale=${scale}></calcite-icon>`;
var CSS = {
  fieldSet: "fieldset",
  star: "star",
  fraction: "fraction",
...
var _Rating = class _Rating extends LitElement {
  constructor() {
    super();
    this.emit = false;
  ...
  render() {
    var _a;
    const countString = (_a = this.count) == null ? void 0 : _a.toString();
    return InteractiveContainer({ disabled: this.disabled, children: html`<span class=${safeClassMap(CSS.wrapper)}>${this.labelText && InternalLabel({ labelText: this.labelText, onClick: this.onLabelClick, required: this.required, tooltipText: this.messages.required }) || ""}<fieldset class=${safeClassMap(CSS.fieldSet)} .disabled=${this.disabled}><legend class=${safeClassMap(CSS.visuallyHidden)}>${this.messages.rating}</legend>${this.starsMap.map(({ average, checked, fraction, hovered, id, partial, selected, value, tabIndex }) => {
...
@esri/calcite-components/dist/components/calcite-rating/customElement.js:
  ...
  v3.3.3 *)
*/
//# sourceMappingURL=customElement-HTBUAFNN.js.map
