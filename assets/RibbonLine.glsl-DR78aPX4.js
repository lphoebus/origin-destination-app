...
return worldToScreenPerDistanceRatio / segmentDistanceToCamera;
  ...
  `),s.uniforms.add(new bi("stipplePatternTexture",n=>n.stippleTexture),new q("stipplePatternPixelSizeInv",n=>1/Rt(n))),e.stippleOffColorEnabled&&s.uniforms.add(new Ce("stippleOffColor",n=>xa(n.stippleOffColor))),a.include(qe),s.code.add(c`float getStippleSDF(out bool isClamped) {
...
return isClamped ? floor(antiAliasedResult + 0.5) : antiAliasedResult;
  ...
  `)}function Rt(a){const e=a.stipplePattern;return e?_a(a.stipplePattern)/e.pixelRatio:1}const Pa=.4,Et=64,wa=Et/2,za=wa/5,Oa=Et/za,ws=.25;function Da(a,e){const t=a.vertex,i=e.hasScreenSizePerspective;et(t),t.uniforms.get("markerScale")==null&&t.constants.add("markerScale","float",1),t.constants.add("markerSizePerLineWidth","float",Oa).code.add(c`
...
vTransitionType = timeStamps.w;`);const{animation:o}=e;o===3&&n.constants.add("decayRate","float",2.3),n.code.add(c`
    float getTrailOpacity(float x) {
      ${Aa(o)}
    }`),n.uniforms.add(new q("timeElapsed",l=>l.timeElapsed),new q("trailLength",l=>l.trailLength),new q("speed",l=>l.animationSpeed),new Ce("timingOptions",l=>ve(La,l.startTime,l.endTime,l.fadeInTime,l.fadeOutTime))),n.code.add(c`float fadeIn(float x) {
...
vec2(vStippleDistanceLimits.x, 1e34);`)),s.main.add(c`
    ...
    }`),e.fragment.include(zi,a),e.include(Oi,a),n.include(Di),n.main.add(c`discardBySlice(vpos);
